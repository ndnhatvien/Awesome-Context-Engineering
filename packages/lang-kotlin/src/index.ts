...
const GRAMMAR_MODULE = 'tree-sitter-kotlin';
...
class Runtime implements LanguageRuntime {
  ...
  async getParser(language: string): Promise<Parser | null> {
    ...
    try {
      ...
      console.warn(`[plugin-kotlin] setLanguage failed for ${language}: ${error.message}`);
      return null;
    }
  }
  ...
  private async loadGrammar(): Promise<TreeSitterLanguage | null> {
    ...
    try {
      ...
      } else if (exported?.kotlin) {
        grammar = exported.kotlin;
      }

      if (!grammar) {
        console.warn(`[plugin-kotlin] Could not extract grammar from module ${GRAMMAR_MODULE}`);
        return null;
      }
      ...
      const error = err as { message?: string };
      console.warn(
        `[plugin-kotlin] Failed to load grammar from ${GRAMMAR_MODULE}: ${error.message}`,
      );
      return null;
    }
  }
