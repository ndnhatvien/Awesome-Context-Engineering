...
const LANGUAGE = 'cpp';
const GRAMMAR_MODULE = 'tree-sitter-cpp';
...
class Runtime implements LanguageRuntime {
  ...
  async getParser(language: string): Promise<Parser | null> {
    ...
    try {
      ...
      console.warn(`[plugin-cpp] setLanguage failed for ${language}: ${error.message}`);
      return null;
    }
  }
  ...
  private async loadGrammar(): Promise<TreeSitterLanguage | null> {
    ...
    try {
      ...
      } else if (exported?.cpp) {
        grammar = exported.cpp;
      }

      if (!grammar) {
        console.warn(`[plugin-cpp] Could not extract grammar from module ${GRAMMAR_MODULE}`);
        return null;
      }
      ...
      const error = err as { message?: string };
      console.warn(`[plugin-cpp] Failed to load grammar from ${GRAMMAR_MODULE}: ${error.message}`);
      return null;
    }
  }
}
