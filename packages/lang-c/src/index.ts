...
const LANGUAGE = 'c';
const GRAMMAR_MODULE = 'tree-sitter-c';
...
class Runtime implements LanguageRuntime {
  ...
  async getParser(language: string): Promise<Parser | null> {
    ...
    try {
      ...
      console.warn(`[plugin-c] setLanguage failed for ${language}: ${error.message}`);
      return null;
    }
  }
  ...
  private async loadGrammar(): Promise<TreeSitterLanguage | null> {
    ...
    try {
      ...
      if (!grammar) {
        console.warn(`[plugin-c] Could not extract grammar from module ${GRAMMAR_MODULE}`);
        return null;
      }
      ...
      const error = err as { message?: string };
      console.warn(`[plugin-c] Failed to load grammar from ${GRAMMAR_MODULE}: ${error.message}`);
      return null;
    }
  }
}
