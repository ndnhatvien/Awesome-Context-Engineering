...
export class Indexer {
    ...
    for (let batchIdx = 0; batchIdx < batches.length; batchIdx++) {
      ...
      for (let fi = 0; fi < batch.length; fi++) {
        ...
        for (let ci = 0; ci < file.chunks.length; ci++) {
          ...
          // （OCE 移植）抽取 chunk 内符号，写入 symbol_occurrences 供精确符号召回
          fileSymbols.push(
            ...extractChunkSymbols(
              record.display_code,
              record.language,
      ...
      for (const upsertBatch of upsertBatches) {
        ...
        // （OCE 移植）同步 symbol_occurrences：先清空该批文件旧符号，再写入新符号
        if (successFiles.length > 0 && isSymbolTableInitialized(db)) {
          try {
            const pathsToDelete = successFiles.map((file) => file.path);
            const symbols = upsertBatch.flatMap((file) => file.symbols);
            batchDeleteFileSymbols(db, pathsToDelete);
            batchUpsertSymbols(db, symbols);
          } catch (err) {
            const error = err as { message?: string };
            logger.warn({ error: error.message }, '符号表更新失败（向量索引已成功）');
          }
        }
  ...
  private async deleteFiles(db: Database.Database, paths: string[]): Promise<void> {
    ...
    // 删除精确符号索引（OCE 移植）
    if (isSymbolTableInitialized(db)) {
      batchDeleteFileSymbols(db, paths);
    }
