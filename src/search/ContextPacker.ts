...
import { coverageSelect } from './coverage.js';
...
export class ContextPacker {
  ...
  async pack(chunks: ScoredChunk[]): Promise<Array<{ filePath: string; segments: Segment[] }>> {
    ...
    // （OCE 移植）覆盖率优先选择：贪心两段填充，先把每文件最高分段纳入，
    // 再补充第二段，兼顾"文件覆盖面"与"预算"。预算与每文件上限复用现有配置。
    if (this.config.coverageEnabled) {
      chunks = coverageSelect(chunks, {
        maxPerPath: this.config.maxSegmentsPerFile,
        maxChars: this.config.maxTotalChars,
        overlapThreshold: this.config.coverageOverlapThreshold,
      });
      if (chunks.length === 0) return [];
    }
